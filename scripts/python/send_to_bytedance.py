...
# ByteDance menggunakan Volcano Engine API (Doubao)
...
def analyze_pmi_logistics(image_path):
    """Analisis gambar logistik PMI dengan ByteDance Doubao Vision API"""
    ...
    # Setup client - ByteDance Volcano Engine API (compatible with OpenAI format)
    client = OpenAI(
        api_key=API_KEY,
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="doubao-vision-pro-32k",  # ByteDance Doubao Vision model
            messages=[
                {
                    "role": "user",
                    "content": [
...
def main():
    ...
    # Periksa API key
    if not API_KEY:
        print("\n⚠️  WARNING: ARK_API_KEY not found in .env file!")
        print("Get your API key from: https://console.volcengine.com/ark")
        return
