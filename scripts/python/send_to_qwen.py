...
# Qwen menggunakan DashScope API dari Alibaba Cloud
...
def analyze_pmi_logistics(image_path):
    """Analisis gambar logistik PMI dengan Qwen VL API"""
    ...
    # Setup client - Qwen uses DashScope API compatible with OpenAI format
    client = OpenAI(
        api_key=API_KEY,
    ...
    try:
        response = client.chat.completions.create(
            model="qwen-vl-max",  # Qwen Vision Language model
            messages=[
                {
                    "role": "user",
                    "content": [
...
def main():
    ...
    # Periksa API key
    if not API_KEY:
        print("\n⚠️  WARNING: DASHSCOPE_API_KEY not found in .env file!")
